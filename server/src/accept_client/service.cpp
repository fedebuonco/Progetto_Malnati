...
void Service::HandleClient(std::shared_ptr<asio::ip::tcp::socket> sock) {
    ...
    switch (request_message.type_) {
        ...
        case 2:{//TREE & HASH Request
            ...
            // And for the tree we retrive its stored MD5

            //TODO Implement DB and retrieve hash according to this functions

            // std::string hash = RetrieveTreeHash(user,tree);
            // tree_result.AddElement("Hash", hash);
